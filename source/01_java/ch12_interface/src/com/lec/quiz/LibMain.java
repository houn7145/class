...
public class LibMain {
	public static void main(String[] args) {
		...
		String bTitle, borrower = null, checkOutDate = null; // 사용자에게 받을 책이름, 대출인, 대출일
		do {
			...
			case 1: // 대출로직 : 1.책이름입력 2.책조회 3.책상태확인 4.대출인입력 5.대출일입력 6.대출메소드 호출
				...
				if (idx == books.length) {
					...
					// 3.책상태확인
					if (books[idx].getState() == BookLib.STATE_BORROWED) { // 대출중
						books[idx].checkOut(borrower, checkOutDate); // 대출중 메소드 호출
					} else { // 4.대출인입력
