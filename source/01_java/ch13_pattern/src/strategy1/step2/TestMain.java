	...
	//	Robot[] robots = {new SuperRobot(), new StandardRobot(), new LowRobot()};
		...
		for(Robot robot : robots) {
			...
			if(robot instanceof SuperRobot) {
				SuperRobot tempRobot = (SuperRobot)robot; //객체의 명시적인 형변환 
				tempRobot.actionFly();
				tempRobot.actionMissile();
				tempRobot.actionKnife();
			}else if(robot instanceof StandardRobot) {
				StandardRobot tempRobot = (StandardRobot)robot; //객체의 명시적인 형변환 
				tempRobot.actionFly();
				tempRobot.actionMissile();
				tempRobot.actionKnife();
			}else if(robot instanceof LowRobot) {
				LowRobot tempRobot = (LowRobot)robot; //객체의 명시적인 형변환 
				tempRobot.actionFly();
				tempRobot.actionMissile();
				tempRobot.actionKnife();	
			}
