...
// 기능(메소드 : deposit(예금), withdraw(출금), infoPrint(정보출력)
public class Account {
	...
	public void withdraw(long money)throws Exception { // 잔액이 있을 경우만 인출하기, 잔액부족시 오류 출력
		if(balance>=money) {
			balance =+ money;
			System.out.println(money + "원 인출하여 잔액 :" + balance);
		}else {
			// ch15장에서는 강제 예외 발생 처리
	...
	//  "\""+accountNo+"\" " + ownerName+"님 잔액 : " + balance + "원";
