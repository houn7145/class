...
public class SelectAllMySQL {
	public static void main(String[] args) {
		...
		try {
			...
			conn = DriverManager.getConnection(url, "root", "mysql"); // 2단계 DB 연결객체 생성
			stmt = conn.createStatement(); // 3단계 SQL 전송 객체
			rs	 = stmt.executeQuery(sql); // 4단계 SQL 전송 + 5단계 SQL결과 받기
			// 6단계 결과를 받아 원하는 로직 수행
			System.out.println("사번\t이름\t직책\t\t상사사번\t\t입사일\t급여\t상여\t부서번호");
			while(rs.next()) {
				int pno = rs.getInt("pno"); // 타이틀명 입력
