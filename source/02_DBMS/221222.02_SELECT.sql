...
SELECT * FROM EMP WHERE ENAME < 'D'; -- 이름이 D보다 작은(A,B,C)인 사원 출력
...
SELECT ENAME, SAL, SAL*12 ANNUASAL -- (3)번째
    FROM EMP                    -- (1)번째
    WHERE ANNUALSAL >= 2400;    -- (2)번째 WHERE절에는 필드 별칭 사용 불가
SELECT ENAME, SAL, SAL*12 ANNUALSAL -- (3)번째
    FROM EMP                    -- (1)번째
    WHERE SAL*12 >= 2400;       -- (2)번째
    -- ex4. 연봉이 10,000 이상인 직원의 ENAME, SAL, 연봉(연봉순으로 정렬) 출력
SELECT ENAME, SAL, SAL*12 연봉   -- (3)번째
    FROM EMP                    -- (1)번째
    WHERE SAL*12 >= 2400        -- (2)번째
    ORDER BY 연봉;               -- (4)번째
    -- ex5. 10번 부서(DEPTNO)이거나 JOB이 MANAGER인 사원의 모든 필드 출력
...
SELECT ENAME, SAL, COMM, SAL*12+COMM 연봉 FROM EMP; 
    -- 산술연산의 결과에 NULL이 포함되어 있으면 결과는 무조건 NULL로 출력됨
...
SELECT ENAME, NVL(TO_CHAR (MGR), 'CEO') MGR FROM EMP; -- 타입을 동일하게 맞춰줌

-- 6. 연결연산자(||) : 필드나 문자를 연결하는데 사용
SELECT ENAME || '은 ' || JOB EMPLOYESS FROM EMP;
    --ex. 모든 사원에 대하여 'SMITH : ANNUAL YEAR SALARY = XXXX' 포맷으로 출력 (연봉=SAL*12+COMM)
SELECT ENAME || ': ANNUAL YEAR SALARY = ' || (SAL*12+NVL(COMM, 0)) MESSAGE FROM EMP;
...
SELECT ENAME, SAL, HIREDATE FROM EMP ORDER BY SAL DESC, HIREDATE DESC; -- 급여 내림차순(같은경우 최신 입사일순)
SELECT ENAME, SAL, HIREDATE FROM EMP ORDER BY ENAME; -- ABC순으로 정렬
SELECT ENAME, SAL, HIREDATE FROM EMP ORDER BY HIREDATE; -- 먼저 입사한 순서대로 정렬
