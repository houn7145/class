-- [ IV ] 단일행함수
-- 함수 = 단일행함수 + 그룹함수(집계함수) 
SELECT HIREDATE, TO_CHAR(HIREDATE, 'YYYY-MM-DD') FROM EMP; -- 단일행씩 수행
SELECT ENAME, INITCAP(ENAME) FROM EMP; -- 첫 글자만 대문자로 출력, 단일행함수(INPUT 1행 -> OUTPUT 1행)
...
-- ★ 단일행함수 종류 : 숫자관련함수, 문자관련함수, 날짜관련함수, 형변환함수, NVL(), ETC...
-- (1) 숫자관련함수
    -- DUAL 테이블 : 오라클에서 1행 1열짜리 더미 테이블을 제공
SELECT * FROM DUAL;
DESC DUAL;

SELECT ABS(-1) FROM DUAL; -- 절대값 출력
SELECT FLOOR(34.56788) FROM DUAL; -- 소수점 아래를 버림
SELECT TRUNC(34.5678) FROM DUAL; -- 소수점 아래를 버림
SELECT FLOOR(34.56788*10)/10 FROM DUAL; -- 소수점 한자리 아래를 버림
SELECT TRUNC(34.5678, 1)FROM DUAL; -- 소수점 한자리 아래를 버림
SELECT FLOOR(34.5678/10)*10 FROM DUAL; -- 일의 자리에서 버림
SELECT TRUNC(34.5678, -1) FROM DUAL; -- 일의 자리에서 버림
    -- ex. EMP테이블에서 이름, 급여(십의 자리에서 버림)
SELECT ENAME, TRUNC(SAL, -2) FROM EMP;
SELECT CEIL(34.5678) FROM DUAL; -- 소수점에서 올림
SELECT ROUND(34.5678) FROM DUAL; -- 소수점에서 반올림
SELECT ROUND(34.5678*100)/100 FROM DUAL; -- 소수점 세번째 자리에서 반올림
SELECT ROUND(34.5678, 2) FROM DUAL; -- 소수점 두번째짜리까지 나오도록 반올림
SELECT ROUND(34.5678/10)*10 FROM DUAL; -- 일의 자리에서 반올림
SELECT ROUND(34.5678, 1) FROM DUAL; -- 소수점 첫번째자리까지 나오도록 반올림
    -- 나머지 연산자
SELECT MOD(9, 2) FROM DUAL; -- 9를 2로 나눈 나머지 9%2
    -- ex. 홀수년도에 입사한 사원의 모든 정보 출력
SELECT * FROM EMP WHERE MOD(TO_CHAR(HIREDATE, 'RR'), 2) = 1;

-- (2) 문자관련함수
-- 1. 대소문자 관련
SELECT INITCAP('WELCOME TO ORACLE') FROM DUAL; -- 앞의 알파벳만 대문자로
...
SELECT ENAME||'는 ' ENAME, JOB || '다' JOB FROM EMP;

-- 3. SUBSTR(STR, 시작위치, 문자갯수) : STR을 시작위치부터 문자갯수만큼 문자 추출(시작위치가 1부터, 
                                            -- 시작위치가 음수면 끝부터 자리수를 셈))
   -- SUVSRTB(SRT, 시작바이트위치, 문자바이트수)
SELECT SUBSTR('ORACLE', 3, 2) FROM DUAL; -- 3번째 글자부터 2글자 : 'AC' 출력
SELECT SUBSTRB('ORACLE', 3, 2) FROM DUAL; -- 3번째 바이트부터 2바이트 : 'AC' 출력
SELECT SUBSTR('데이터베이스', 4, 3) FROM DUAL; -- 4번째 글자부터 3글자 : '베이스' 출력
SELECT SUBSTRB('데이터베이스', 4, 3) FROM DUAL; -- 4번째 바이트부터 3바이트 : '이' 출력
    -- O R A C L E
    -- 1 2 3 4 5 6(양수위치)
    ---6-5-4-3-2-1(음수위치)
    -- 02-717-9999
SELECT SUBSTR('WELCOME TO ORACLE', -1, 1) FROM DUAL; -- -1번째 글자부터 1글자 : 'E' 출력
SELECT SUBSTR('ORACKE', -2, 2) FROM DUAL; -- 마지막 글자 2글자 : 'RE' 출력
SELECT SUBSTR(123, 2, 1) FROM DUAL; -- 숫자도 가능(숫자를 문자로 바꿔서 2번째 숫자부터 1글자 : '2' 출력
    
    -- ex. 9월에 입사한 사원의 모든 필드 출력 'RR/MM/DD'
SELECT * FROM EMP WHERE SUBSTR(HIREDATE, 4, 2) = '09'; -- DATE형도 가능
SELECT * FROM EMP WHERE SUBSTR(TO_CHAR(HIREDATE,'RR/MM/DD'), 4, 2) = '09';
    -- ex. 9일에 입사한 사원이 모든 필드 출력 'RR/MM/DD'
SELECT * FROM EMP WHERE SUBSTR(HIREDATE, -2, 2) = '09';
SELECT * FROM EMP WHERE SUBSTR(TO_CHAR(HIREDATE,'RR/MM/DD'), -2, 2) = '09';

-- 4. LENGTH(SRT) : SRT의 글자수
   -- LENGTHB(STR) : SRT의 바이트 수
SELECT LENGTH('ABCD') FROM DUAL; -- 굴자수 출력
SELECT LENGTHB('ABCD') FROM DUAL; -- 바이트수 출력
SELECT LENGTH('오라클') FROM DUAL; -- 글자수 '3' 출력
SELECT LENGTHB('오라클') FROM DUAL; -- 바이트수 '9' 출력

-- 5. INSTR(STR, 찾을문자) : STR에서 첫번째 문자부터 찾을문자가 나오는 위치(없으면 0)
   -- INSTR(STR, 찾을문자, 시작위치) : STR에서 시작위치 문자부터 찾을문자가 나오는 위치
SELECT INSTR('ABCABC', 'B') FROM DUAL; -- 처음 나오는 B의 위치 '2' 출력
SELECT INSTR('ABCABC', 'B', 3) FROM DUAL; -- 3번째 글자부터 찾아서 처음 나오는 B의 위치 '5' 출력
SELECT INSTR('ABCABC', 'B', -3) FROM DUAL; -- 뒤 3번째부터 찾아서 처음 나오는 B의 위치 '2' 출력
SELECT INSTR('ABCABCABC', 'B', -3) FROM DUAL; -- 뒤 3번째부터 찾아서 처음 나오는 B의 위치 '5' 출력

    -- ex. 9월에 입사한 사원의 모든 필드 출력(INSTR 이용) 'RR/MM/DD'
SELECT * FROM EMP WHERE INSTR(HIREDATE, '09', 4) = 4;
    -- ex. 9일에 입사한 사원의 모든 필드 출력(INSTR 이용)
SELECT * FROM EMP WHERE INSTR(HIREDATE, '09', 7) = 7;    
    -- ex. 9일이 아닌 다른 날에 입사한 사원의 모든 필드 출력(INSTR 이용)
...
SELECT LPAD('ORACLE', 10, '#') FROM DUAL; -- ####ORACLE 출력
SELECT RPAD('ORACLE', 10, '*') FROM DUAL; -- ORACLE**** 출력
...
-- 8. REPLACE(STR, 바꿔야할문자, 바꿀문자) : STR에서 바꿔야할문자를 바꿀문자로 교체
...
-- 2. 날짜계산 : 오라클 타입(문자, 숫자, 날짜 / 숫자, 날짜는 연산 가능)
ALTER SESSION SET NLS_DATE_FORMAT = 'RR/MM/DD HH24:MI:SS';
ALTER SESSION SET NLS_DATE_FORMAT = 'RR/MM/DD';
SELECT SYSDATE-1 어제이시간, SYSDATE, SYSDATE+1 내일이시간 FROM DUAL; -- 1 = 24시간
...
-- 3. MONTHS_BETWEEN(시점1, 시점2) : 두 시점간의 개월 수(시점1을 큰 시점)
    -- ex. 이름, 입사일, 근무월수
SELECT ENAME, HIREDATE, TRUNC(MONTHS_BETWEEN(SYSDATE, HIREDATE)) MONTH FROM EMP;
    -- ex. 이름, 입사한 이후 받은 돈(SAL은 1년에 12번, COMM은 2번)을 출력
...
SELECT ENAME, HIREDATE, ADD_MONTHS(HIREDATE, 6)FROM EMP;
    -- 입사일이 81/08/30이면 수습 종료사점 81/02/28
    
-- 5. NEXT_DAY(특정시점, '요일') : 특정시점부터 처음 도래하는 요일
SELECT NEXT_DAY(SYSDATE, '토') FROM DUAL;

-- 6. LAST_DAY(특정시점) : 특정시점의 말일
SELECT LAST_DAY(SYSDATE) FROM DUAL;
    -- ex. 이름, 입사일, 첫월급날(월급날은 말일)
SELECT ENAME, HIREDATE, LAST_DAY(HIREDATE) FROM EMP;

-- 7. ROUND(날짜, XX) : 날짜 반올림 (XX:YEAR(가까운 1/1), MONTH(가까운 1일), DAY(가까운 일요일) 
    -- TRUNC(날짜, XX) : 날짜 버림
SELECT ROUND(34.5678, 2) FROM DUAL;
SELECT ROUND(SYSDATE,'YEAR') FROM DUAL; -- 가까운 1/1
SELECT ROUND(SYSDATE,'MONTH') FROM DUAL; -- (1~15일까지는 이번달 1일 / 16일 이후는 다음달 1일)
SELECT ROUND(SYSDATE,'DAY') FROM DUAL; -- 가까운 일요일
SELECT ROUND(SYSDATE) FROM DUAL; -- 가까운 0시 0분

SELECT TRUNC(SYSDATE,'YEAR') FROM DUAL; -- 같은 해 1/1
...
SELECT TRUNC(SYSDATE) FROM DUAL; -- 오늘 0시 0분
    -- ex1. 이름, 입사일, 첫월급날(11일) : 10일이전: 이번달 11일 // 11일이후면 다음달 11일 
                        -- ROUND의 기준 15,16을 10,11로
SELECT ENAME, HIREDATE, ROUND(HIREDATE+5, 'MONTH')+10 FROM EMP; -- 월급날이 11일
    -- ex2. 월급날 10일 : 9일까지는 이번달, 10일부터는 다음달
SELECT ENAME, HIREDATE, ROUND(HIREDATE+6, 'MONTH')+9 FROM EMP; -- 월급날이 10일
    -- ex3. 월급날 25일 : 24일까지는 이번달, 25일부터는 다음달
SELECT ENAME, HIREDATE, ROUND(HIREDATE-9, 'MONTH')+24 FROM EMP; -- 월급날이 25일

-- (4) 형변환함수 : TO_CHAR(문자로 변환), TO_DATE(날짜로 변환)
-- 1. TO_CHAR(날짜형, '출력형식')
    -- YYYY / RR / MM / DD / DY 요일
...
SELECT TO_DATE('20221227 14:53', 'YYYYMMDD HH24:MI') FROM DUAL;
    -- ex. 81/05/01 ~ 83/05/01 사이에 입사한 직원의 모든 필드 출력
SELECT * FROM EMP 
...
-- 5. NVL(NULL일 수 있는 데이터, NULL을 대신 할 값) - 매개변수 2개의 타입이 일치해야 실행됨
    -- ex1. 사원이름, 직속상사이름(직속상사가 없으면 CEO로 출력)
SELECT W.ENAME, NVL(M.ENAME, 'CEO')
    FROM EMP W, EMP M
    WHERE W.MGR = M.EMPNO(+);
    
    -- ex2. 사원이름, 직속상사의 사번(직속상사가 없으면 CEO로 출력)
SELECT ENAME,NVL(TO_CHAR(MGR), 'CEO') MGR
    FROM EMP;
    
-- 6. ETC
-- (1) EXTRACT : 년도, 월, 일만 추출하고자 할 때
SELECT HIREDATE, EXTRACT(YEAR FROM HIREDATE) FROM EMP; -- 년도만 숫자로 출력
...
SELECT ENAME, JOB, HIREDATE, TO_CHAR(HIREDATE, 'DY"요일"') DAY
    FROM EMP;
