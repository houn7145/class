...
SELECT DEPTNO, MAX(SAL) FROM EMP; -- 단일행과 다중행은 같이 쓸 수 없음
...
-- ★ 2. GROUP BY 절
    -- ex1. 부서번호별 최대 급여, 평균 급여 출력 (부서번호별 정렬)
...
-- GROUP BY절은 컬럼의 별칭을 사용할 수 없음(ORDER BY절은 가능)
    ...
    -- ex2. 부서이름별 최대 급여, 평균 급여 출력 (평균 급여순 정렬)
SELECT DNAME, MAX(SAL), AVG(SAL)
    ...
    -- ex3. SAL이 5000미만이 사원에 대해서만 부서번호별 사원수, 최대급여, 최소급여, 평균급여 출력(부서번호순 정렬)
...
-- ★ 3. HAVING 절 : 그룹함수의 결과 조건
    -- ex1. 부서번호별 평균 급여(평균급여가 2000 이상인 부서만 출력)
SELECT DEPTNO 부서번호, AVG(SAL)
    FROM EMP
    GROUP BY DEPTNO
    HAVING AVG(SAL) > 2000 -- 그룹함수의 조건
    ORDER BY 부서번호; -- 필드의 별칭은 ORDER BY절에만 사용 가능
    
    -- ex2. SAL이 5000미만인 사원에 대해서 부서명별 최소급여, 최대급여, 평균급여
        --단 평균급여가 1800이상인 부서명에 대해서 평균급여순으로 정렬 후 출력
