...
SELECT ENAME, MAX(SAL) FROM EMP GROUP BY ENAME; -- 다 나옴
...
-- 서브쿼리의 종류(2) 다중행 서브쿼리(서브쿼리의 실행결과가 2행이상) : IN, >(ALL, ANY(SOME)),EXISTS
    -- ex. JOB이 MANAGER인 사원의 부서이름 출력
SELECT DEPTNO 
    FROM EMP 
    WHERE JOB = 'MANAGER'; -- 서브쿼리 결과가 2행이상
SELECT DNAME
    FROM DEPT
    WHERE DEPTNO IN (SELECT DEPTNO FROM EMP WHERE JOB = 'MANAGER'); -- 메인쿼리

-- ★ 2. 단일행 서브쿼리
    -- ex1. SCOTT과 동일한 부서번호에서 근무하는 사원의 이름과 급여 출력
SELECT DEPTNO 
    FROM EMP 
    WHERE ENAME = 'SCOTT'; -- 서브쿼리
SELECT ENAME, SAL 
    ...
    -- ex2. SCOTT과 동일한 근무지에서 근무하는 사원의 이름과 급여 출력
-- 데이터 추가 (DALLAS 50번 부서의 HONG 사원)
...
SELECT HIREDATE, ENAME
    ...
    -- ex5. 최초입사일, 최초입사자, 최근입사일, 최근입사자 출력
...
SELECT 
    ...
    -- ex6. SCOTT과 같은 부서에 근무하는 사람들의 급여합
SELECT DEPTNO 
    FROM EMP 
    WHERE ENAME = 'SCOTT'; -- 서브쿼리
SELECT SUM(SAL) 
    ...
    -- ex7. SCOTT과 동일한 JOB를 가진 사원의 모든 필드
SELECT JOB 
    FROM EMP 
    WHERE ENAME = 'SCOTT'; -- 서브쿼리
SELECT * 
    ...
    -- ex8. DALLAS에서 근무하는 사원의 이름과 부서번호
SELECT DEPTNO 
    FROM DEPT 
...
SELECT ENAME, DEPTNO 
    ...
    -- ex9. KING이 직속상사인 사원의 이름과 급여 출력
SELECT EMPNO 
    FROM EMP 
    WHERE ENAME = 'KING'; -- 서브쿼리
SELECT ENAME, SAL 
    FROM EMP 
    WHERE MGR = (SELECT EMPNO FROM EMP WHERE ENAME = 'KING'); -- 메인쿼리
    
    -- ex10. 평균급여 이하로 받는 사원의 이름과 급여 출력
SELECT AVG(SAL) 
    FROM EMP; -- 서브쿼리
SELECT ENAME, SAL 
    FROM EMP
    ...
    -- ex11. 평균급여 이하로 받는 사원의 이름과 급여, 평균급여 출력
SELECT ENAME, SAL, ROUND((SELECT AVG(SAL) FROM EMP)) AVG_SAL
    FROM EMP
    WHERE SAL <= (SELECT AVG(SAL) FROM EMP);

    -- ex12. 평균급여 이하로 받는 사원의 이름과 급여, 평균급여와의 차이 출력
...
-- (1) IN : 서브쿼리 결과 중 하나라도 일치하면 참
    -- ex1. 부서별 입사일이 가장 늦은 사람의 이름, 입사일, 부서번호 출력
...
SELECT ENAME, HIREDATE, DEPTNO
    ...
    -- ex2. 급여가 3000이상 받는 사원이 소속된 부서의 사원들의 모든 필드 출력
SELECT DEPTNO 
    FROM EMP 
...
-- (2) ALL : 서브쿼리 결과가 모두 만족하면 참
    -- ex1. 30번 부서직원 모두의 급여보다 큰 직원의 모든 필드 출력
SELECT SAL 
    FROM EMP 
    WHERE DEPTNO = 30; -- 950, 1250, 2850, 1500, 1600 서브쿼리
SELECT * 
    FROM EMP
...
-- (3) ANY(SOME) : 서브쿼리 결과가 하나라도 만족하면 참
    -- ex1. 30번 부서직원 한명의 급여보다 큰 직원의 모든 필드 출력
SELECT SAL 
    FROM EMP 
    WHERE DEPTNO = 30; -- 다중행 단일열 서브쿼리
SELECT *
    FROM EMP
...
-- 급여가 3000이상인 사람들 중 연봉 등급을 나누어서 해당 등급별 최고 연봉을 받는 사람들의 사번, 이름, 직업, 입사일, 급여, 급여등급을 출력
...
SELECT HIREDATE, CEIL(EXTRACT(MONTH FROM HIREDATE)/3) 분기 FROM EMP; -- 분기
SELECT TO_CHAR(HIREDATE, 'Q') 분기, MAX(SAL) FROM EMP GROUP BY TO_CHAR(HIREDATE, 'Q'); -- 서브쿼리
SELECT HIREDATE, TO_CHAR(HIREDATE, 'Q') 분기, EMPNO, ENAME, JOB, MGR, HIREDATE, SAL, COMM
    FROM EMP
...
-- SALESMAN 일부 어떤 한 사원보다 급여를 많이 받는 사원들의 이름과 급여와 직급(담당 업무)를 출력하되 영업 사원도 출력(ANY)
SELECT ENAME, SAL, JOB 
    FROM EMP
