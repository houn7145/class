...
select database();
show databases;
show tables; -- select * from tab; 현재 들어와 있는 데이터베이스 내의 테이블
-- ★ ★ ★ DDL
create table emp(
...
-- MAJOR (mCODE;시퀀스pk 학과번호, mName 학과이름, mOffice 학과사무실) 
-- STUDENT (sNo 학번, sName 이름, mCode 학과번호)
create table major(
	mCode int primary key auto_increment, -- auto_increment 필드인 경우 타입을 int
	mName varchar(30),
	mOffice varchar(30)
);
create table student(
    ...
    sName varchar(30),
	mCode int references major(mCode) -- 외래키 제약조건은 옆에다 쓰면 안됨
);
select * from student;
select * from major;
insert into major (mName, mOffice) values ('컴공', '203호'); -- 컴공번호 1
insert into major (mName, mOffice) values ('빅데이터', '303호'); -- 빅데이터 2
insert into major (mName, mOffice) values ('디자인', '403호'); -- 디자인 3
select * from major;
...
create table student(
	...
	mCode int,
	primary key (sNo),
    foreign key(mCode) references major(mCode) -- 외래키 제약 조건은 반드시 아래에만 사용가능
);
insert into major (mName, mOffice) values ('컴공', '203호'); -- 컴공번호 1
insert into major (mName, mOffice) values ('빅데이터', '303호'); -- 빅데이터 2
insert into major (mName, mOffice) values ('디자인', '403호'); -- 디자인 3
...
select pno, pname, pay, pay*12+ifnull(bonus,0) from personal; -- nvl
...
from personal p, (select dno, avg(pay) avgpay
	from personal
    group by dno) a
    where pay > avgpay;
 -- 서브쿼리 이용
...
select pname, pay, dname
	from division d, personal p, (select startdate from personal
...
select pname, pay, avg
	from personal p, (select dno, avg(pay) avg
	from personal
	group by dno) a
    where p.dno=a.dno;
...
select pname, pay, dname, avgpay
	from division d, personal p, (select dno, avg(pay) avgpay
	from personal
    group by dno) a
    where d.dno = p.dno;
    
-- ★ ★ ★ Oracle에서의 단일행함수와 다른 부분
select curdate(); -- 현재날짜만
...
select now();
select year(sysdate()), month(sysdate()), day(now()), hour(now()),
			minute(now()), second(now()); -- month는 1월부터
...
select pname, monthname(startdate) from personal; -- 이름, 입사한 달
...
-- date_format(문자, 포맷) => 날짜 
		-- 포맷 : %Y 2023(년도 4자리) %y 23(년도 2자리)
		-- 		%m (01월, 02월,...) %c (1,2....) %M 월이름(January), %d 짧은월이름(Jan)
        -- 		%d (01, 02, 03,..) %e (1, 2, 3...)
...
select pname, format(pay, 0) from personal; -- 세자리마다 ,
...
select pname, pay from personal order by pay desc limit 0, 5; -- 1등부터 5개 출력
select pname, pay from personal order by pay desc limit 5, 5; -- 6등부터 5개 출력
select pname, pay from personal order by pay desc limit 6, 3; -- 7등부터 3개 출력
