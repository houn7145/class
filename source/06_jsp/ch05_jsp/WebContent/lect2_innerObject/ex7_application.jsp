...
<%
	...
	String absolutePath = application.getRealPath("."); // 현재 프로젝트가 실행되는 절대 경로(tomcat)
%>
...
<%
	...
	// 1. 스트림 객체 생성(파일열기 - 기본스트림->보조스트림) 2. 파일읽고 화면에 출력 3. 스트림 객체 닫기 (파일닫기)
	...
	try{
		...
		while(true){
			String line = br.readLine(); // 한줄씩 읽음. 파일의 끝이면 null을 반환
			if(line == null) {
				break;
			}
			out.println(line + "<br>");
