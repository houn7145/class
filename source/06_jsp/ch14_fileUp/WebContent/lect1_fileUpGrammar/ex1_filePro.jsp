...
<body>
	<%
		String path = request.getRealPath("fileUpFolder"); // 첨부할 파일이 저장될 서버(WAS) 폴더
		// out.print(path);
		...
		try{
			...
			while(paramNames.hasMoreElements()){
				...
				System.out.println("파라미터 이름 : " + param + 
								  "/서버에 저장된 파일 이름 : " + filename + 
								  "/사용자가 첨부한 파일 이름 : " + originalFilename);
			}
		}catch(IOException e){
			System.out.println(e.getMessage());
		}
		
		// 서버에 업로드 한 후 소스폴더로 file copy -- 프로젝트 완성 후 삭제
		InputStream is = null;
		OutputStream os = null;
		try{
			File serverFile = new File(path + "/" + filename);
			//if(filename != null){
			if(serverFile.exists()){ // 파일이 존재하면
				is = new FileInputStream(serverFile); // 서버에 업로드 된 파일
				os = new FileOutputStream("C:/houn/source/06_jsp/ch14_fileUp/WebContent/fileUpFolder/" + filename); // 소스폴더로 복사될 파일
				...
				System.out.println("서버에 업로드한 파일을 소스폴더로 복사완료");
			}
		}catch(Exception e){
			System.out.println(e.getMessage());
		}finally{
	...
	<img src="<%=conPath %>/fileUpFolder/<%=filename %>" alt="첨부 안함">
	  <%}else{ 
		out.println("<h2>첨부 안함 : filename= " + filename + ", 오리지널= " + originalFilename + "</h2>");
	    }%>
</body>
</html>