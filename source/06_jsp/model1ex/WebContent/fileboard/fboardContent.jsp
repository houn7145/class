...
<body>
	...
	<%
		...
		FileboardDto fDto = fDao.getFileboard(fnum); // 글 번호로 글 상세점보 가져오기
	%>
	<table>
		<caption><%=fnum %>번 글 상세보기</caption>
		<tr><th>글 번호</th><td><%=fDto.getFnum() %></td></tr>
		<tr><th>글 제목</th><td><%=fDto.getFsubject() %></td></tr>
		<tr>
			<th>작성자</th>
			<td>
		...
		<tr>
			<td colspan="2">
				<!-- <button>수정</button> 글쓴이의 id : fDto.getCid() 로그인한 사람의 id : session id
				<button>답변</button>
				<button>삭제</button>
				<button>목록</button> -->
			<%
				...
				if(fDto.getCid().equals(cid)){ // 글쓴이 id와 로그인 한 사람의 id가 같을 경우만 수정가능
			%>		
