	...
	<div id="content_form">
		<table>
			...
			<% // fboardList.jsp 또는 fboardList.jsp?pageNum=1
				...
				int totCnt = fDao.getFileBoardTotalCnt(); // db에 등록된 글 갯수
				if(totCnt == 0){  // db에 글이 없음
					out.println("<tr><td colspan='7'>등록된 글이 없습니다</td></tr>");
				}else if(dtos.isEmpty()){
					out.println("<tr><td colspan='7'>해당 페이지의 글이 없습니다</td></tr>");
				}else{
					for(FileboardDto dto : dtos){ // 글번호, 작성자, 제목, 메일, ip, 작성일, 조회수
			%>		
					<tr>
						<td><%=dto.getFnum() %></td>
						<td><%=dto.getCname() %></td>
						<td class="left">
							...
							<%if(dto.getFfilename() != null){ %> <!-- 파일첨부를 하면 -->
