...
<body>
	...
	<%
		// fboardWriteForm.jsp - 원글쓰기(fsubject, fcontent, ffilename, fpw)
		//										(fref, fre_step, fre_level) - 0으로 넘김
		// fboardWriteForm.jsp?fnum=1&pageNum=9 - 1번글에 대한 답글 쓰기(fsubject, fcontent, ffilename, fpw)
		...
		FileboardDto fDto = new FileboardDto(); // 빈 dto객체 생성(fnum, fref, fre_step, fre_level은 전부 0)
		if(request.getParameter("fnum") != null){ // fnum이 null이 아니면 fnum번글 답글을 쓰러 옴
			fnum = Integer.parseInt(request.getParameter("fnum"));
			FileboardDao fDao = FileboardDao.getInstance();
			fDto = fDao.getFileboard(fnum); // 원글에 대한 정보(fref, fre_step, fre_level)
		}
	%>
	<form action="<%=conPath %>/fileboard/fboardWritePro.jsp" method="post" enctype="multipart/form-data">
		...
		<input type="hidden" name="fre_level" value="<%=fDto.getFre_level() %>"> <!-- 원글이면 0을 넘김 -->
		<table>
			<tr>
				<th>글제목</th>
				<td>
					<input type="text" name="fsubject" required="required" autofocus="autofocus"
						value="<%
							if(fDto.getFsubject() != null) {
								out.println("답)" + fDto.getFsubject());
							}
						%>">
				</td>
			</tr>
