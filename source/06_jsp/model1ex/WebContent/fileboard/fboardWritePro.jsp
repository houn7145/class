...
<body>
	<%
		...
		if(ffilename != null){
			...
			try{
				...
				os = new FileOutputStream("C:/houn/source/06_jsp/model1ex/WebContent/fileboardUpload/" + ffilename);
		...
		// 파라미터 정보(pageNum, fnum, fsubject, fcontet, fpw, fref, fre_step, fre_level)가져오기
		...
		// fnum이 0이면 dao 원글쓰기 메소드 호출
		// num이 0이 아니면 dao 답변글쓰기 메소드 호출
		int result;
		if(fnum == 0){ // 원글쓰기
	...
	%>		
			<script>
				...
				location.href='<%=conPath%>/fileboard/fboardList.jsp?pagrNum=<%=pageNum%>';
			</script>
	<% 	}else{%>
			<script>
